...
class Challenge7 extends Component {
  ...
  delimiterhandle = () => {
    ...
    if (dublicate) {
      ...
      delimiter = delimiter.map(delimiter => {
        for (let a = 0; a < delimiter.length; a++) {
          if (/([^a-z0-9])/.test(delimiter[x])) {
            delimiter[1] = "\\" + delimiter[x];
          }
        }
  ...
  render() {
    return (
      <StyledCard>
        <StyledTitle>
          Part 8: Support multiple delimiters of any length.
        </StyledTitle>
